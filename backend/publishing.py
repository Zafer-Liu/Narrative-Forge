...
def _public_project(project, root, project_dir):
    ...
    for index, source in enumerate(scenes):
        ...
        scene = {
            ...
            "duration": int(source.get("duration", 8)) if str(source.get("duration", 8)).isdigit() else 8,
            ...
            "storyNodeKey": str(source.get("storyNodeKey") or ""),
            "shotInNode": int(source.get("shotInNode", 1)) if str(source.get("shotInNode", 1)).isdigit() else 1,
            "shotsInNode": int(source.get("shotsInNode", 1)) if str(source.get("shotsInNode", 1)).isdigit() else 1,
